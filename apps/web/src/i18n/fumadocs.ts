...
const labels: Record<Locale, readonly [string, string, string, string, string, string]> = {
  'zh-hans': ['深色', '浅色', '跟随系统', '打开侧栏', '关闭侧栏', '没有小节'],
  'zh-hant': ['深色', '淺色', '跟隨系統', '開啟側欄', '關閉側欄', '沒有小節'],
...
export function fumadocsTranslations(locale: Locale): Record<string, string> {
  ...
  return {
    ...
    'Copy Text(code block)(aria-label)': ui.copy,
    'Copy Anchor Link(heading anchor)(aria-label)': ui.copy,
    'Copy Link(accordion)(aria-label)': ui.copy,
  };
}
