...
export function sitemapXml(documentIds: string[], releaseVersions: string[] = [], updated: Record<string, string> = {}): string {
  ...
  const urls = paths.flatMap((path) => localeDefinitions.map(({ id }) => {
    ...
    const date = updated[`${id}/${path}`];
    const modified = date && /^\d{4}-\d{2}-\d{2}$/.test(date) ? `<lastmod>${date}</lastmod>` : '';
    return `<url><loc>${escapeXml(canonicalOrigin + localePath(id, path))}</loc>${modified}${alternate}<xhtml:link rel="alternate" hreflang="x-default" href="${escapeXml(canonicalOrigin + localePath('en', path))}"/></url>`;
  }));
