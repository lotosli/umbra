...
pub struct ForgedLeaf {
    ...
    /// Ephemeral leaf private key DER for later TLS `CertificateVerify` signing.
...
pub fn cert_key(shared: &[u8], session_id: &[u8]) -> Result<[u8; 32], RealityError> {
    let key = hkdf_sha256(CERT_KEY_SALT, shared, session_id, 32)
...
pub fn cert_mac(
    ...
    let key = cert_key(shared, session_id)?;
    hmac_sha256(&key, leaf_spki_der)
