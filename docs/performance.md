...
The [completion audit](../openspec/changes/optimize-multiclient-throughput/completion-audit.md) remains open: the native QUIC admission correction is implemented in the working source but still awaits final artifact/deployment reconciliation; group scheduling is not established by memory caps, and production bottleneck observations do not yet cover all modes/directions. The deployed release is not a claim that these remaining plan requirements have been completed.
