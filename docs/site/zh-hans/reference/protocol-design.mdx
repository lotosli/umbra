...
title: "协议设计"
description: "完整协议设计：TLS 1.3、REALITY 认证、Vision、QUIC、密码学与连接状态机。"
section: "reference"
order: 17
version: "1.0.0-alpha"
...
这份技术参考完整保留 Umbra 的协议设计：一套 Rust 客户端与服务端，以及组件、接口、线格式和设计取舍。内容涵盖目标设计与历次修订；部署方法以使用文档为准，§17 的单 crate 布局与 §18 的依赖列表属于早期草案。

[使用指南](/zh-hans/docs/getting-started/quick-start/) · [当前架构](/zh-hans/docs/concepts/architecture/)
...
<ProtocolDiagram locale="zh-hans" name="architecture" />
...
接收端依据实际消费与RTT增长窗口，在授信前获得进程/凭据组预算；已承诺信用不可撤回。FIN/RST按累计位置结算并保留取消安全。未认证回落和ClientHello构造不变。配置、内存口径和测量限制见[吞吐说明](https://github.com/lotosli/umbra/blob/main/docs/performance.md)。
...
精确线格式、上限、拒绝/提交状态、EOF/取消及向量见[已批准的 wire 规范](https://github.com/lotosli/umbra/blob/main/docs/vision-runtime-wire-v2.md)。
真实运行时测试使用独立 rustls 端点，确认双向256 KiB业务完整、线上后缀与内层密文逐字节一致，且切换后外层seal/open计数停止。
实际Mac与服务器0.0.7线上请求也已验证拼接；详见[验收记录](https://github.com/lotosli/umbra/blob/main/openspec/changes/integrate-vision-runtime/verification.md)。
...
以下为早期单 crate 布局草案；当前多 crate workspace 见[架构说明](/zh-hans/docs/concepts/architecture/)。草案采用单二进制 + 子命令（`umbra server|client|keygen`）。组件→模块：
...
**服务器分派/握手**
<ProtocolDiagram locale="zh-hans" name="server" />

**客户端**
<ProtocolDiagram locale="zh-hans" name="client" />
