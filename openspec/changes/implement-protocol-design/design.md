...
### Runtime Reliability Follow-up (2026-09-13)

The user explicitly requested resolving `mux target setup` failures, preserving multiplexing, increasing usable concurrency, and verifying a usable client/server product. This follow-up revises the earlier single-TCP-outer design while retaining its approved bounded lifecycle, coordinated establishment, target isolation, and no-payload-replay requirements. It does not amend the wire protocol or relax authentication.

- Admit TCP opens using the actual per-session receive-credit capacity, not only the nominal stream-count constant. Default 256 KiB reservations within 8 MiB mean 32 streams per outer. Coordinate at most four accepting compatible outers for 128 default live streams, at most four additional retiring outers, and at most 128 admission waiters. Keep per-outer memory bounds unchanged; do not solve saturation by unbounded allocation.
- Reserve capacity atomically before selecting an outer. Reclaim reservations only when inner state is reclaimed, or on terminal cancellation/failure. Full sessions must not strand capacity-eligible requests while another bounded outer can be established.
- Distinguish pool admission, SYN transmission, and target acknowledgement. Allow the client acknowledgement wait 25 seconds, beyond the server's 15-second target guard. Retire an outer from new admissions when opening progress is lost; preserve still-useful sibling streams while retirement capacity permits and never replay application payload. Retiring outers do not consume accepting slots. At the total eight-outer bound, a new accepting outer may replace the oldest retiring outer, terminating its remaining streams explicitly rather than waiting forever; healthy accepting outers are never evicted by this rule. Normal target rejection remains stream-local.
- Resolve TCP targets with a bounded DNS deadline and interleave IPv4/IPv6 candidates. Stagger address attempts instead of letting one unreachable address consume the entire setup budget. Keep at most four attempts in flight and sixteen candidates, cancel/reap losers, and preserve a single address's overall connection budget. Stage errors contain no destination, authentication, or payload data.
- Return a standard SOCKS general-failure response when CONNECT setup fails, rather than closing without a reply. This improves failure signaling but is not a substitute for connection and recovery fixes.
- Report only non-sensitive TLS bridge stages/error classes so a dropped record translation task is no longer indistinguishable from a stalled target.
- The historical intermittent incident's initial trigger is not claimed to be proven. Acceptance must exercise the fixed failure modes and actual product path with multiplexing enabled, including ordinary sites, held concurrent connections, and recovery.
