...
| 457-489 | Server and client config schema. | `crates/umbra-core/src/config.rs`; `crates/umbra/src/main.rs`; `examples/{server,client}.toml`. | `scenario_complete_server_config_loads`, `scenario_complete_client_config_loads`, CLI override tests. | Implemented. |
...
| 582-593 | Tests: auth tamper/replay, cert/PQ, RFC vectors, fingerprint, handshake, probing, TLS-in-TLS, timing, RST/QUIC, real env guidance. | Unit, integration, proptest, fuzz targets, and testkit under `crates/*/tests` and `fuzz/`. | `cargo nextest run --workspace` 201 passed; `cargo xtask coverage` 90.10%. | Implemented. |
...
- `cargo nextest run --workspace`: 201 passed.
- `cargo xtask coverage`: 201 passed, line coverage 90.10%.
