...
The system SHALL seal a 16-byte REALITY plaintext into a 32-byte `legacy_session_id` using AES-128-GCM, an HKDF-SHA256 key and nonce derived from X25519 shared secret, and `HELLO0` as AAD.
...
The system SHALL validate version, timestamp window, allowed short id, and replay cache before accepting an opened token.
