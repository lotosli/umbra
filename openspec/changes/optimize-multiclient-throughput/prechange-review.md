...
## R14 — Credential-group ready-work scheduling review

The authenticated runtime spawns independent outer, TLS record, mux driver, target and Quinn driver tasks. Tokio schedules tasks without credential identity; the pool in resources.rs only enforces memory ceilings. Thus one group with eight continuously ready tasks can receive roughly eight times the poll opportunities of a group with one equivalent ready task. First measure that task-count effect, then verify the same ready-work fixture with group scheduling.

Use a shared readiness gate around authenticated Tokio task boundaries, with round robin across credential groups and then queued tasks. Keep futures in their original Tokio tasks: their existing JoinHandle abort/join ownership remains intact. A proxy waker queues only ready work. A permit covers one synchronous cooperative Future::poll, is returned on Pending/Ready/drop, and never remains occupied while waiting for socket input or credit. Permit capacity follows the Tokio worker count; a sole ready group can use all permits. Coalesce wakes and remove cancelled queue entries. The global mutex only coordinates task readiness and permit handoff, never executes I/O or frame processing. Do not impose a fixed Mbps rate, per-group single-thread actor, per-DATA shared lock or a new cryptographic/runtime implementation. At most one gate wraps a Tokio task; child tasks get separate gates. Unauthenticated classification/fallback keep the original executor path.

Apply the same group scheduler to TCP Vision relay, TCP mux outer/driver/record workers/target tasks, native QUIC's runtime-spawned drivers and stream tasks, and authenticated UDP target readers. Validate 8-to-1 ready-task competition, paused work, wake coalescing, cancellation and multicore permit borrowing, plus existing full runtime tests and a representative byte-copy workload to quantify added scheduling cost. Diagnostics from the gate describe poll opportunities and waiting, not equal bytes or CPU time across unlike work.

The readiness fixture measured 80,000 vs 10,000 baseline polls. Review of the first gate iteration found that releasing before re-queuing a self-woken single-task group produced 60,001 vs 29,999: enqueue that already-ready work before the permit handoff, yielding 45,001 vs 44,999 in the same fixture. A further allocation review found that deleting the empty group deque after every grant would allocate again on its next wake. Retain and reuse the deque while any task in that group exists, and free it when the last task closes; group metrics remain bounded by configured credential identities. These are corrections within R14, not weakened fairness assertions.

## R15 — Data-path observation review

The current public snapshots expose scheduler work and total commitments but cannot distinguish pending target I/O from outer socket backpressure or exhausted DATA credit. Benchmarks had to reconstruct those causes separately. Add opt-in diagnostic collection (`diagnostics_interval_secs`, zero by default), with anonymous per-outer mode/group identifiers, independent transport/target read/write byte and pending-poll counters, observed await duration where the interface supports it, sampled mux/native credit and buffering state, and group admission/growth refusal counters. Transport bytes include protocol overhead; target-read bytes are prefetched business bytes, not proof of remote application delivery. Do not label them HTTP goodput or infer CPU utilization from task wall time.

Use connection-local atomic counters; the registry is touched only at registration, close and snapshot. Keep at most 128 completed snapshots and retain only active observations otherwise. Sample credit state during existing driver progress at most every 100ms, avoiding a new per-DATA stream scan or global lock. Native UDP callbacks report bytes and pending polls; duration remains unavailable where no owned read/write future spans the wait. Periodic reporting emits only typed modes, opaque indexes and numeric counters. Budgets/scheduler remain enforceable when diagnostics are disabled. Verify actual pending read/write, partial progress, snapshot cleanup and bounded completion history, plus a real multi-client runtime snapshot containing the expected groups and bytes. No target, credential, SNI, address or session id enters diagnostic records.

The full run exposed an existing injected-outer fixture race: its fake mux server dropped the entire transport immediately after sending one stream's FIN, before the client had necessarily completed its own FIN/credit flush. The payload assertion succeeded, but the client outcome could correctly report BrokenPipe. This fixture tests injected routing and successful closure, not abrupt transport loss. Keep the fake outer alive until the client completes, retaining all payload, target, close and success assertions. Do not change production error handling to ignore that race. Abrupt close/cancellation remain covered by the dedicated mux tests. Diagnostic reporting uses async stderr writes so a slow log sink does not synchronously block the accept loop.
