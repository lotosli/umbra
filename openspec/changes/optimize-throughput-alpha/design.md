...
Enable aes_armv8 and polyval_armv8 for aarch64 targets through checked-in Cargo target rustflags, retaining library runtime detection. Add feature unification for AES and POLYVAL zeroization before retaining expanded contexts. Reusable AEAD contexts accept caller-owned payloads and explicit nonces; no secret Debug/Clone. Clear failed output and destroy all cached key/hash state on drop or explicit clearing. Keep stateless wrappers for compatibility and reference tests.
