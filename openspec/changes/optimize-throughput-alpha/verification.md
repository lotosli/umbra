...
[Umbra 1.0.0-alpha](https://github.com/lotosli/umbra/releases/tag/v1.0.0-alpha) is published as a non-draft prerelease without replacing stable latest. All four macOS/Linux binary attachments, SHA256SUMS and BUILD-INFO.json completed upload. The tag resolves to `a7e63079efe03991243f550426ab6f6d5b2c0386`; running `cargo xtask dist` at that source produced exactly the same four hashes as the installed artifacts. Only task/verification documentation differs from the implementation commit that passed all tests.

The tag's [Dist workflow](https://github.com/lotosli/umbra/actions/runs/34939498767) was also prevented from starting by the account billing/spending restriction (zero steps on macOS/Linux/Windows, release job skipped). The independently built and already deployed local artifacts were uploaded directly. At the user's subsequent instruction to stop testing and publish, no further tests were run; remaining work was publication and status confirmation only.
